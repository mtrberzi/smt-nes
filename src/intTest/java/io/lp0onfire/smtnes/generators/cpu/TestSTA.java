...
public class TestSTA {
  ...
  @Test
  public void testSTA_ABY() throws IOException {
    // opcode 99
    
    // run the program
    // $0000: 99 90 29  STA $2990,Y
    // with A = $9B, Y = $73
    // and assert that we see a write to $2A03 with data = $9B
    ...
    exprs.addAll(reg.apply(new CodeGenerator(){
      ...
      public List<SExpression> generateCode(Map<String, Symbol> inputs,
        ...
        exprs.add(new Assertion(new EqualsExpression(Y, new HexConstant("73"))));
  ...
  @Test
  public void testSTA_INX() throws IOException {
    // opcode 81
    ...
    ROMBurner burner = new ROMBurner("TestROM");
    burner.write(0x000, 0x81);
    burner.write(0x001, 0x40);
    burner.write(0x052, 0x03);
    burner.write(0x053, 0x2A);
    // reset vector
    burner.write(0xFFD, 0x00); // high byte
    burner.write(0xFFC, 0x00); // low byte
  ...
  @Test
  public void testSTA_INY() throws IOException {
    // opcode 91
    ...
    ROMBurner burner = new ROMBurner("TestROM");
    burner.write(0x000, 0x91);
    burner.write(0x001, 0x40);
    burner.write(0x040, 0xF1);
    burner.write(0x041, 0x29);
    // reset vector
    burner.write(0xFFD, 0x00); // high byte
    burner.write(0xFFC, 0x00); // low byte
    ...
    exprs.addAll(reg.apply(new CodeGenerator(){
      ...
      public List<SExpression> generateCode(Map<String, Symbol> inputs,
        ...
        Symbol Y = outputs.get("CPU_Y");
        exprs.add(new BitVectorDeclaration(Y, new Numeral("8")));
        exprs.add(new Assertion(new EqualsExpression(Y, new HexConstant("12"))));
